...
if SERVER then
	...
	resource.AddFile("materials/vgui/ttt/icon_manhack.png")
    ...
    hook.Add("EntityTakeDamage","ManhackDamage", function( hack, dmginfo )
        ...
        if inflictor.ManhackExplosion then
            if inflictor.ManhackExplosion.Controller:ManhackOwner(hack) then
                dmginfo:SetDamage(0)
            else
        ...
        if inflictor.Controller then
            if inflictor.Controller:HaxOwner(hack) then
                dmginfo:SetDamage(0)
            else
    ...
    function meta:HaxOwner(hack)
        return table.HasValue(self.ThrownHax, hack)
    end

end

hook.Add( "EntityTakeDamage", "ManHackDamage", function( ent, dmginfo )
    if !dmginfo:GetAttacker():IsValid() then return end
